...
public class PostgreSqlDatasetContext implements DatasetContext {
     ...
     */
    public PostgreSqlDatasetContext(
            DatabaseLoginCredentials loginCredentials, DatabasePreferences preferences, boolean logging) {
    ...
    public ReleasableIterator<EntityContainer> iterateBoundingBox(
        ...
        rowCount = jdbcTemplate.update(
                "CREATE TEMPORARY TABLE bbox_nodes ON COMMIT DROP AS" + " SELECT * FROM nodes WHERE (geom && ?)",
        ...
        resultSets.add(
                new UpcastIterator<EntityContainer, WayContainer>(new WayContainerIterator(wayDao.iterate("bbox_"))));
