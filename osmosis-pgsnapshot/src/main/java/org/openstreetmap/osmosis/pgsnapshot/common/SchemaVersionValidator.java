    ...
    private void validateDBVersion(int expectedVersion) {
        if (preferences.getValidateSchemaVersion()) {
            ...
            dbVersion = Objects.requireNonNull(
                    jdbcTemplate.queryForObject(SELECT_SQL, Integer.class), "Schema version was not found");

            if (dbVersion != expectedVersion) {
                throw new OsmosisRuntimeException("The database schema version of " + dbVersion
