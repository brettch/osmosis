...
public class PostgreSqlWriter implements Sink, EntityProcessor {
    ...
    private void initialize() {
        if (!initialized) {
            ...
            bulkWayStatement =
                    statementContainer.add(dbCtx.prepareStatement(wayBuilder.getSqlInsert(INSERT_BULK_ROW_COUNT_WAY)));
            ...
            singleRelationTagStatement =
                    statementContainer.add(dbCtx.prepareStatement(relationTagBuilder.getSqlInsert(1)));
            ...
            singleRelationMemberStatement =
                    statementContainer.add(dbCtx.prepareStatement(relationMemberBuilder.getSqlInsert(1)));
    ...
    private void flushWayTags(boolean complete) {
        while (wayTagBuffer.size() >= INSERT_BULK_ROW_COUNT_WAY_TAG) {
            ...
            for (int i = 0; i < INSERT_BULK_ROW_COUNT_WAY_TAG; i++) {
                prmIndex =
                        wayTagBuilder.populateEntityParameters(bulkWayTagStatement, prmIndex, wayTagBuffer.remove(0));
            }
