...
public abstract class DatabaseTaskManagerFactory extends TaskManagerFactory {
    ...
    protected DatabaseLoginCredentials getDatabaseLoginCredentials(TaskConfiguration taskConfig) {
        ...
        if (doesArgumentExist(taskConfig, DatabaseConstants.TASK_ARG_AUTH_FILE)) {
            ...
            authLoader = new AuthenticationPropertiesLoader(
                    new File(getStringArgument(taskConfig, DatabaseConstants.TASK_ARG_AUTH_FILE)));
        ...
        loginCredentials.setHost(
                getStringArgument(taskConfig, DatabaseConstants.TASK_ARG_HOST, loginCredentials.getHost()));
        loginCredentials.setDatabase(
                getStringArgument(taskConfig, DatabaseConstants.TASK_ARG_DATABASE, loginCredentials.getDatabase()));
        loginCredentials.setUser(
                getStringArgument(taskConfig, DatabaseConstants.TASK_ARG_USER, loginCredentials.getUser()));
        loginCredentials.setPassword(
                getStringArgument(taskConfig, DatabaseConstants.TASK_ARG_PASSWORD, loginCredentials.getPassword()));
        loginCredentials.setForceUtf8(
                getBooleanArgument(taskConfig, DatabaseConstants.TASK_ARG_FORCE_UTF8, loginCredentials.getForceUtf8()));
        loginCredentials.setProfileSql(getBooleanArgument(
                taskConfig, DatabaseConstants.TASK_ARG_PROFILE_SQL, loginCredentials.getProfileSql()));
        loginCredentials.setDbType(getStringArgument(
                taskConfig,
                DatabaseConstants.TASK_ARG_DB_TYPE,
                loginCredentials.getDbType().toString()));
        loginCredentials.setPostgresSchema(getStringArgument(
                taskConfig,
                DatabaseConstants.TASK_ARG_POSTGRES_SCHEMA,
    ...
    protected DatabasePreferences getDatabasePreferences(TaskConfiguration taskConfig) {
        ...
        // command line.
        preferences.setValidateSchemaVersion(getBooleanArgument(
                taskConfig,
                DatabaseConstants.TASK_ARG_VALIDATE_SCHEMA_VERSION,
                preferences.getValidateSchemaVersion()));
        preferences.setAllowIncorrectSchemaVersion(getBooleanArgument(
                taskConfig,
