    ...
    private int parseTask(String[] programArgs, int offset) {
        ...
        while (i < programArgs.length) {
            ...
            if (equalsIndex >= 0) {
                ...
                if (PipelineConstants.IN_PIPE_ARGUMENT_PREFIX.equals(argName)
                ...
                if (defaultArgIndex >= 0) {
                    throw new OsmosisRuntimeException(
                            "Only one default (un-named) argument can exist per task.  Arguments " + (i + 1) + " and "
                                    + (defaultArgIndex + 1) + " have no name.");
                }
        ...
        taskConfigList.add(new TaskConfiguration(taskId, taskType, pipeArgs, taskArgs, defaultArg));
