...
public class FileBasedSort<T extends Storeable> implements Closeable {
    ...
    private ReleasableIterator<T> iteratePersisted(int nestLevel, long beginChunkIndex, long chunkCount) {
        ...
        persistentIterator = new PersistentIterator<T>(
                serializationFactory, iterate(nestLevel, beginChunkIndex, chunkCount), "emtb", useCompression);
        ...
        try {
            ...
            if (chunkCount <= MAX_MERGE_SOURCE_COUNT) {
                for (int i = 0; i < chunkCount; i++) {
                    sources.add(chunkedEntityStore.iterate(beginChunkIndex + i));
                }
                ...
                for (long subFirstChunk = beginChunkIndex;
                    ...
                    if (((nestLevel + 1) % MAX_MEMORY_SORT_DEPTH) == 0 && subChunkCount > 1) {
                        sources.add(iteratePersisted(nestLevel + 1, subFirstChunk, subChunkCount));
                    } else {
                        sources.add(iterate(nestLevel + 1, subFirstChunk, subChunkCount));
                    }
                }
            }
