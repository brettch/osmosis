...
public class TaskRegistrar {
    ...
    private void loadBuiltInPlugins() {
        ...
        try {
            for (URL pluginConfigurationUrl : Collections.list(
            ...
            throw new OsmosisRuntimeException(
                    "Unable to load the plugins based on " + pluginResourceName + " resources.");
        }
    }
    ...
    private void loadJPFPlugins() {
        ...
        PluginDescriptor core =
                pluginManager.getRegistry().getPluginDescriptor("org.openstreetmap.osmosis.core.plugin.Core");
        ...
        for (Iterator<Extension> it = point.getConnectedExtensions().iterator(); it.hasNext(); ) {
            ...
            try {
                ...
                throw new OsmosisRuntimeException(
                        "Cannot load JPF-plugin '" + ext.getId() + "' for extensionpoint '" + ext.getExtendedPointId()
                                + "'",
                        e);
            }
        }
    ...
    private List<PluginLocation> gatherJpfPlugins() {
        File[] pluginsDirs = new File[] {
            new File("plugins"),
            new File(System.getProperty("user.home") + "/.openstreetmap" + File.separator + "osmosis" + File.separator
                    + "plugins"),
            ...
            try {
                for (int i = 0; i < plugins.length; i++) {
                    ...
                    if (location != null) {
                        ...
                        LOG.warning(
                                "JPF Plugin " + plugins[i].getAbsolutePath() + " is malformed and cannot be loaded.");
                    }
                }
