...
public class PipeTasks {
    ...
    public void putTask(String taskId, Task task) {
        ...
        if (LOG.isLoggable(Level.FINE)) {
            LOG.fine("Task \"" + taskId + "\" produced unnamed pipe stored at level " + defaultTasks.size()
    ...
    public Task retrieveTask(String taskId, Class<? extends Task> requiredTaskType) {
        ...
        if (LOG.isLoggable(Level.FINE)) {
            LOG.fine("Task \"" + taskId + "\" consumed unnamed pipe stored at level " + defaultTaskCount
                    + " in the default pipe stack.");
        }
